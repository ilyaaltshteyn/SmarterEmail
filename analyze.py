# This script has an Emails class which is initialized with a list of emails.
# The class can filter out unnecessary emails and run analyses on them.
from textstat import lexicon_count, sentence_count, flesch_reading_ease, flesch_kincaid_grade
...
class Emails():
    ...
    def analyze_one(self, email):
        ...
        print email
        print 'LEX COUNT: ', lexicon_count(email)
        # self.sent_counts.append(sentence_count(email))
        print 'SENT COUNT: ', sentence_count(email)
        # self.flesch_scores.append(flesch_reading_ease(email))
        print 'READING EASE: ', flesch_reading_ease(email)
        # self.flesch_kincaid_grades.append(flesch_kincaid_grade(email))
        print 'READING GRADE: ', flesch_kincaid_grade(email)
